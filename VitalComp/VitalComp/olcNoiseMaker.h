...
template<class T>
class olcNoiseMaker{
	public: olcNoiseMaker(std::wstring sOutputDevice, unsigned int nSampleRate = 44100, unsigned int nChannels = 1, unsigned int nBlocks = 8, unsigned int nBlockSamples = 512){
		Create(sOutputDevice, nSampleRate, nChannels, nBlocks, nBlockSamples);
	}
	public:	~olcNoiseMaker(){
		Destroy();
	}
	public: bool Create(std::wstring sOutputDevice, unsigned int nSampleRate = 44100, unsigned int nChannels = 1, unsigned int nBlocks = 8, unsigned int nBlockSamples = 512){
		m_bReady = false;
	...
	private: FTYPE(*m_userFunction)(int, FTYPE);

	private: size_t m_nSampleRate;
	private: size_t m_nChannels;
	private: size_t m_nBlockCount;
	private: size_t m_nBlockSamples;
	private: size_t m_nBlockCurrent;

	private: T* m_pBlockMemory;
	private: WAVEHDR* m_pWaveHeaders;
	private: HWAVEOUT m_hwDevice;

	private: std::thread m_thread;
	private: std::atomic<bool> m_bReady;
	private: std::atomic<size_t> m_nBlockFree;
	private: std::atomic<FTYPE> m_dGlobalTime;
