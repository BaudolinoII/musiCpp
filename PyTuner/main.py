...
class Application(tk.Frame):
	...
	def update_hot_data(self):
		...
		self.hot_data = osc.operation_c(input_data, self.curr_sim, self.rate_sample)
	...
	def update_cold_data(self):
		...
		if len(method_ops):#Si existe contenido por sumar
			...
			self.cold_data = osc.operation_c(method_data, self.curr_sim, self.rate_sample)
	...
	def refresh_screen(self,*args):
		...
		if(self.show_four.get()):
			...
			if(self.show_hot.get() and len(self.f_hot_data) and (self.curr_expr.get() != 'Operation')):
			...
			if(self.show_hot.get() and len(self.hot_data) and (self.curr_expr.get() != 'Operation')):
	...
	def expr_listener(self,*args):
		...
		self.value_e.set(value='0.0')
		self.tag_a.set(value='Amplitude')
		self.tag_b.set(value='Frecuency')
		self.tag_c.set(value='Amp_Resonare')
		self.tag_d.set(value='Freq_Resonare')
		self.tag_e.set(value='Origin')

		if(self.curr_expr.get() == "Square"):
			self.tag_c.set(value='WorkRate')
			self.tag_d.set(value='IsPulse?')
			self.iv = [-1.0, 1.0, 0.0, 0.0, 0]
		elif(self.curr_expr.get() == "Saw"):
			self.txb_e.config(state=tk.NORMAL)
			self.tag_e.set(value='Precision')
		elif(self.curr_expr.get() == "Noise"):
			self.tag_b.set(value='Seed')
			self.tag_c.set(value='Begin')
			self.tag_d.set(value='Duration')
	...
	def update_lb_meth(self, *args):
		...
		for x in self.xman.read_all_ops(self.curr_note.get()):
			reson = "";
			if(float(x[3]) != 0):
				reson = " + {:.1f}*{:.2f}*sin({:.2f}wt)".format( float(x[2]), float(x[3]), float(x[4]))
			args = "{:.2f}{}({:.2f}wt{}) + {:.2f}".format(float(x[1]), self.get_co(x[0]), float(x[2]), reson, float(x[5]))

			if(x[0] == 'q'):
				name = self.get_co(x[0])
				if(float(x[4]) == 0.0):
					name = "Pulse"
				args = "{:.2f}{}({:.2f}wt) + {:.2f}; WR => {:.1f}%".format(float(x[1]), name , float(x[2]), float(x[5]), (float(x[3]) * 100))
			elif(x[0] == 'w'):
				if(int(x[5]) != 0):
					args = "{:.2f}{}({:.2f}wt{}); Detail => {}".format(float(x[1]), self.get_co(x[0]), float(x[2]), reson, int(x[5]))
				else:
	...
	def get_axis_id_freq(self, *args):
		try:
			self.tag_axis_freq.set('Frecuency {:.2f} Hz'.format(ut.armonic_freq(int(self.axis_id_note.get()))))
		except ValueError:
			pass
	#Controladores
	def set_method(self):
		if (self.curr_expr.get() == 'Operation'):
			return 0
	...
	def ask_for_ga_params(self):
		self.sec_win = tk.Toplevel()
		self.sec_win.title('Generate Harmonics')
		self.sec_win.geometry('300x240')

		lbl_aga_op = tk.Label(self.sec_win, text='Operation type')
		lbl_aga_op.grid(column=0,row=0)
		cbx_aga_ops = ttk.Combobox(self.sec_win, values=["Sine","Square","Triangle","Saw"], textvariable= self.op_ga)
		cbx_aga_ops.set("Sine")
		...
		btn_aga_ga = tk.Button(self.sec_win, text='Generate', command=self.generate_armonics)
		btn_aga_ga.grid(column=0,row=2)
		btn_aga_ex = tk.Button(self.sec_win, text='Cancel', command=self.sec_win.destroy)
		btn_aga_ex.grid(column=1,row=2)
	def ask_for_cf_params(self):
		self.sec_win = tk.Toplevel()
		self.sec_win.title('Generate Tones')
		self.sec_win.geometry('300x240')

		lbl_notes = tk.Label(self.sec_win,text='Fundamental')
		...
		if not self.mode_note.get():
			lbl_tag = tk.Label(self.sec_win,text='Name Tone')
			lbl_tag.grid(column=1, row=0, sticky='N')
			ax_notes = ttk.Combobox(self.sec_win,
			values=self.occ_cat, textvariable=self.axis_tag_note)
		...
		self.chb_hot = tk.Checkbutton(self.sec_win, text="General Template", variable=self.with_temp, onvalue=True, offvalue=False)
		...
		btn_gnr_nts = tk.Button(self.sec_win,text='Generate',command=self.generate_notes)
		btn_gnr_nts.grid(column=0, row=5, sticky='N')
		btn_gnr_nts = tk.Button(self.sec_win,text='Cancel',command=self.sec_win.destroy)
	...
	def generate_notes(self):
		if self.mode_note.get():
			...
			begin = self.occ_cat.index(self.axis_tag_note.get())
			for note in self.occ_cat:
		...
		self.tag_axis_freq.set(value='Frecuency 0.0 Hz')
	...
	def create_vars(self): 
		...
		self.curr_expr = tk.StringVar(value='Operation')
		...
		#Etiquetas Variables
		self.tag_a = tk.StringVar(value='Amplitude')
		self.tag_b = tk.StringVar(value='Frecuency')
		self.tag_c = tk.StringVar(value='Amp_Resonare')
		self.tag_d = tk.StringVar(value='Freq_Resonare')
		self.tag_e = tk.StringVar(value='Origin')
		...
		#Variables Metadatos
		self.value_name = tk.StringVar(value='Instrument')
	...
	def create_widgets(self):
		...
		self.nt_expression.add(self.fr_template, text = "Template")
		self.nt_expression.add(self.fr_method, text = "Method")
		self.nt_expression.grid(column=0, row=1, sticky='NW')

		self.nt_control.add(self.fr_xml, text = "XML")
		self.nt_control.add(self.fr_draw, text = "Graphic")
		...
		self.btn_safe = tk.Button(self.fr_xml, text="Export", command=self.save_xml_file, width=25)
		self.btn_safe.grid(column=0, row=0, sticky='N')
		self.btn_load = tk.Button(self.fr_xml, text="Load", command=self.load_xml_file, width=25)#width=110
		self.btn_load.grid(column=0, row=1, sticky='N')
		self.btn_noise = tk.Button(self.fr_xml, text="Test in Melody", command=self.play_waves, width=25)#width=110
		self.btn_noise.grid(column=0, row=2, sticky='N')
		self.btn_keyb = tk.Button(self.fr_xml, text="Test in Keyboard", width=25)#width=110
		...
		self.btn_agn = tk.Button(self.fr_xml, text="Feel Tones", command=self.ask_for_cf_params, width=25)#width=110
		self.btn_agn.grid(column=1, row=1, sticky='N')
		self.lbl_name = tk.Label(self.fr_xml, text='Instrument Name')
		...
		self.lbl_vol = tk.Label(self.fr_xml, text='Volume')
		...
		self.lbl_durM = tk.Label(self.fr_xml, text='Max Life Time')
		...
		#Opciones del Lienzo
		self.chb_hot = tk.Checkbutton(self.fr_draw, text="Current", fg='red', variable=self.show_hot, onvalue=True, offvalue=False)
		self.chb_hot.grid(column = 0, row = 0, sticky='N')
		self.chb_cold = tk.Checkbutton(self.fr_draw, text="Model", fg='blue', variable=self.show_cold, onvalue=True, offvalue=False)
		...
		self.chb_cold = tk.Checkbutton(self.fr_draw, text="Template", fg='orange', variable=self.show_temp, onvalue=True, offvalue=False)
		...
		#Opciones de audio
		self.btn_audio = tk.Button(self.fr_audio, text="Load", command=self.load_wav_file, width=50)# width=115
		self.btn_audio.grid(column = 0, row = 0, columnspan=2,sticky='N')
		self.btn_audio = tk.Button(self.fr_audio, text="Paly", command=self.play_sound, state=tk.DISABLED, width=50)# width=115
		self.btn_audio.grid(column = 0, row = 1, columnspan=2,sticky='N')
		self.lbl_ab = tk.Label(self.fr_audio, text='Begin Audio')
		...
		self.lbl_at = tk.Label(self.fr_audio, text='Time Audio')
		...
		self.lbl_sb = tk.Label(self.fr_audio, text='Begin Espectre')
		...
		self.lbl_st = tk.Label(self.fr_audio, text='Length Espectre')
		...
		self.lbl_attack = tk.Label(self.fr_template, text = 'Attack')
		...
		self.lbl_decay = tk.Label(self.fr_template, text = 'Decay')
		...
		self.lbl_sustain = tk.Label(self.fr_template, text = 'Sustain')
		...
		self.lbl_release = tk.Label(self.fr_template, text = 'Release')
		...
		self.lbl_pitch = tk.Label(self.fr_template, text = 'Max')
		...
		self.lbl_stable = tk.Label(self.fr_template, text = 'Stable')
		...
		self.lbl_rest = tk.Label(self.fr_template, text = 'Rest')
		...
		#Panel de metodos
		self.btn_add_sum = tk.Button(self.fr_method, text="Add", command=self.set_method)
		self.btn_add_sum.grid(column=1,row=0, sticky='W')
		self.btn_mod_sum = tk.Button(self.fr_method, text="Edit", command=self.mod_method)
		self.btn_mod_sum.grid(column=2,row=0, sticky='W')
		self.btn_rmv_sum = tk.Button(self.fr_method, text="Quit", command=self.del_method)
		self.btn_rmv_sum.grid(column=3,row=0, sticky='W')
		self.btn_gnr_arm = tk.Button(self.fr_method, text="Auto-Harmonics", command=self.ask_for_ga_params, state=tk.DISABLED)
		...
		self.cbx_ops = ttk.Combobox(self.fr_method,values=["Sine","Square","Triangle","Saw","Noise"],textvariable=self.curr_expr)
		self.cbx_ops.set("Operation")
		self.cbx_ops.grid(column=5,row=0, columnspan=2, sticky='N')

		self.cbx_note = ttk.Combobox(self.fr_method,values=self.occ_cat, textvariable=self.curr_note)
		self.cbx_note.config(state=tk.DISABLED)
		...
		self.btn_id = tk.Button(self.fr_method, text='ID Harmonic', command = self.get_armonic_freq)
		...
		self.lbl_dur = tk.Label(self.fr_method, text = 'Duration')
...
def main():
	root.wm_title("PyTuner by JoGEHrt V_1.1.3")
