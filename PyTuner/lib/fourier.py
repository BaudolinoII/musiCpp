...
class Fourier():
	...
	def get_FFT(self, test_data, bs:int = 0, spectre:int = 50000):
		...
		f_test_data = abs(fourier.fft(test_data))[0:test_lenght//2]
		if((spectre + bs) > test_lenght//2):
			spectre = test_lenght // 2 - bs
	...
	def set_sample_area_at_time(self, begin:float=0.0, time:float=-1.0, bs:int = 0, spectre:int=5000):
		...
		self.sam_f_data = abs(fourier.fft(self.sam_data))[0:len(sam_data)//2]
