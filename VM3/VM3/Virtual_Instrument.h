...
class Sound_Model {
    ...
    public: void set_fund(std::string fund) { this->fund = std::stod(fund); }
    public: void set_scale(std::string scale) { this->scale = std::stoi(scale); std::cout << "Checkpoint Reached: Setting scale: " << (int)this->scale << std::endl;
    }
    ...
    public: FTYPE getAmplitude(const FTYPE globalTime, const FTYPE startTime, const FTYPE endTime, const FTYPE dMLT) {
        FTYPE life_time = globalTime - startTime;//Tiempo que lleva viva la nota

        if (globalTime > endTime && endTime > 0.0)//Se ha alcanzado el final programado
    ...
    public: FTYPE getSound(const FTYPE globalTime, const FTYPE dMLT, const Note note, bool& bNoteFinished) {
        FTYPE ampl = getAmplitude(globalTime, note.on, note.off, dMLT);
        bNoteFinished = (ampl <= 0.0);//LA nota acabará si él máxmimo de vida es alcanzado y su amplitud es > 0
        FTYPE sound = 0.0, ds = 0.0;
        //std::cout << "\nScale note: " << (int)note.scale << " " << (int)this->scale << std::endl;
        const int scale_note = (int)note.id + 12 * (note.scale - this->scale);
        for (std::pair<char, FTYPE*> op : this->ops_val)
            sound += Oscillador::osc(op.first , globalTime, op.second[0], Oscillador::scale(scale_note + op.second[1], this->fund, 0), op.second[2], op.second[3], op.second[4]);
        return sound * ampl;
    }
};
class Instrument_xml {
    ...
    public: int load_document(const std::string path) {
        ...
        for (pugi::xml_node note = notes.first_child(); note; note = note.next_sibling()) {
            ...
            sm.set_fund(ops.attribute("fund").value());
            sm.set_scale(ops.attribute("scale").value());
